function [Prior,PriorN,Cond,CondN,KeyCondLabel,KeyCondLabelN,NKeyCondLabel,NKeyCondLabelN,T1]=MLKNN_train_fast(train_data,train_target,Num,Smooth,ProTrain)
...
% lilicheng add
    ...
    NumOfNKeywordAndLabelN=zeros(1,num_class);
    for i=1:num_class
        NumOfNKeywordAndLabelN(i)=sum((ProTrain(:,i)==0)&(train_target(:,i)==0))+1;
    end
%     NumOfNKeywordAndLabelN=sum((ProTrain==0)&(train_target==0),1);
...
%     end
    ...
    T1=lshSimple(9,size(train_data,2),train_data);
    
%     matlabpool open;
    train_target=full(train_target);
...
%         Neighbors{i,1}=index(1:Num);
    ...
    for i=1:num_training
        if(mod(i,10000)==0)
            disp(strcat('calculating neighbours for instance:',num2str(i)));
        end
